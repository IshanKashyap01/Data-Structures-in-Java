...
`N` and `M`, respectively. Your task is to find the pair of elements (one from
each array), such that their absolute (non-negative) difference is the
smallest, and return the min difference.

**Note**: Both the arrays are unsorted, and all array elements are non-negative
integers.
...
`M`, where `N` and `m` are the sizes of array `arr1` and `arr2`, respectively.

The second line of each test case contains `N` space-separated integers
denoting the elements of array `arr1`.

The third line of each test case contains `M` space-separated integers denoting
...
1. The smallest difference pairs are (7, 6) and (5, 6) with an absolute
difference of 1, so the answer is 1.

2. The smallest difference pair is (20, 17) with an absolute difference of 3,
so the answer is 3.
